export function Benefits({ el, idx }: { el: any; idx: number }) {
  return (
    <div className='mt-[18px] flex gap-1 items-center justify-center sm:gap-[18px]'>
      {data.map((_) => (
        <div
          key={_.label}
          className='w-full py-3 px-1 sm:px-[18px] text-center bg-accentGray-400 rounded sm:rounded-lg text-accentGray-300 text-[10px] font-semibold'
        >
          {_.label}
